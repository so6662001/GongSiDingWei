...
# ================= 06 异议 =================
section_page("06", "抗拒解除", "18 个高频异议，抽考 8 条",
             ["第 1、2、4、5、8、18 条必考"])
...
objs = [
    ...
    ("我的客户扫码进来会看到别家，私域客户不就流失了？", "结构性·必考",
     "不许硬辩解。先承认“这担心是对的”，再讲两条现在成立的事实：① 您的老客户现在也在比价，您不给看他也在看，真正决定成交的是他觉不觉得跟您买更稳；② 海报是拓新客的工具，不是发给老客户的（这是我们没讲清楚）。并主动告知产品正在做专属落地页与归属锁定。", True),
]
for chunk_i in range(0, len(objs), 5):
    chunk = objs[chunk_i:chunk_i + 5]
    s = new_slide()
    y = title_bar(s, "异议库", f"高频异议标准答案　{chunk_i + 1}—{chunk_i + len(chunk)} / 18")
...
s = new_slide()
y = title_bar(s, "纪律", "10 条销售红线（违反即追责）")
reds = ["不许承诺线索一定成交 —— 统一口径：线索是询盘，不是订单。",
        ...
        "不许宣称产品尚未具备的能力 —— 尤其“扫码只进您自己的店”，上线前一律不许说。"]
